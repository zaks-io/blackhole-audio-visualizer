...
export function RecordToggleFab({ isRecording, duration, disabled, onToggle }: RecordToggleFabProps) {
  return (
    <button
      onClick={onToggle}
      disabled={disabled}
      ...
      `}
      title={disabled ? 'Connect microphone first' : isRecording ? 'Stop recording' : 'Start recording'}
    >
      {isRecording ? (
        <>

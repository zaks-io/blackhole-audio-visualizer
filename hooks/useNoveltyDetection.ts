...
const DEFAULT_ENERGY_TREND_THRESHOLD = 0.08; // Minimum change to detect trend (lowered from 0.15)
const TREND_WINDOW = 120; // ~2 seconds for trend detection

export function useNoveltyDetection(energyTrendThreshold: number = DEFAULT_ENERGY_TREND_THRESHOLD) {
  ...
  const processFrame = useCallback(
    (spectral: SpectralFeatures, timestamp: number): NoveltyData => {
      ...
      if (energyHistory.length >= TREND_WINDOW) {
        ...
        const oldAvg =
          oldWindow.length > 0
            ? oldWindow.reduce((a, b) => a + b, 0) / oldWindow.length
            : recentAvg;

        const delta = recentAvg - oldAvg;
        data.energyDelta = delta;

        if (delta > energyTrendThreshold) {
          data.energyTrend = "building";
        } else if (delta < -energyTrendThreshold) {
